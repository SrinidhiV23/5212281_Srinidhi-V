...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    *result_count=queries_count;
    int *ans=(int*)malloc(sizeof(int) * queries_count);
    
    for(int q=0; q<queries_count;q++){
        int d =queries[q];
        int *dq=(int*)malloc(sizeof(int) * arr_count);
        int front =0, back=-1;
        int best=0;
        for(int i=0;i<arr_count;i++){
            while(front<=back && dq[front]<=i-d) front++;
            while(front<=back && arr[dq[back]]<=arr[i]) back--;
            dq[++back]=i;
            if(i>=d-1){
                int curmax=arr[dq[front]];
                
                    if(i==d-1) best=curmax;
                    else if(curmax<best)
                    best=curmax;
                }
            }
