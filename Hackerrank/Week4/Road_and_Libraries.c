...
long roadsAndLibraries(int n, int c_lib, int c_road,int m, int cols, int** cities) {
if(c_lib<=c_road){
    return (long)n* c_lib;
}
int *head=(int*)malloc((n+1)*sizeof(int));
int *to=(int*)malloc((2*m+5) * sizeof(int));
int *next=(int*)malloc((2*m+5) * sizeof(int));
int *visited=(char*)calloc(n+1, sizeof(int));
...
for(int i=1;i<=n;i++){
    if(!visited[i]){
        ...
        total+=c_lib+(comp-1)*(long)c_road;
    }
}
free(head);
free(to);
