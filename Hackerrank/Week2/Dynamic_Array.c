...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    for(int i=0;i<queries_rows;i++){
        ...
        if(type==1){
            seqsizes[idx]++;
            seq[idx]=(int *)realloc(seq[idx], seqsizes[idx]* sizeof(int));
            ...
            lastans=seq[idx][pos];
            outputsize++;
            res=(int *)realloc(res, outputsize * sizeof(int));
            res[outputsize-1]=lastans;
        
        }
    }
    *result_count=outputsize;
