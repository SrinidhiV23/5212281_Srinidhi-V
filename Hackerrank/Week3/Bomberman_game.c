 ...
 */
 ...
 static void fill_bombs(char** g, int r, int c){
    for(int i=0;i<r;i++){
        memset(g[i], 'O', c);
    }
 }
 static void fill_denote(char** src, char** dst, int r, int c){
    fill_bombs(dst, r, c);
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
            if(src[i][j]=='O'){
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    *result_count=r;
    ...
    char** grid3 =alloc_grid(r, c);
    char** grid5=alloc_grid(r, c);
    fill_denote(grid, grid3, r , c);
    fill_denote(grid3, grid5, r, c);
